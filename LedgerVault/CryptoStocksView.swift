...
struct CryptoStocksView: View {
    ...
    /// Converts a value in `baseCurrency` to the wallet's native currency.
    private func nativeAccountValue(_ valueInBase: Double, for wallet: APIService.Account) -> Double {
        let base   = baseCurrency.uppercased()
        let target = wallet.base_currency.uppercased()
                        ...
                        ForEach(cryptoWallets) { wallet in
                            Button { selectedAccount = wallet } label: {
                                HStack(spacing: 14) {
                                    ...
                                    Spacer()
                                    VStack(alignment: .trailing, spacing: 4) {
                                        let value = nativeAccountValue(accountValue(for: wallet), for: wallet)
                                        if value > 0 {
                                            Text(fmtValue(value, currency: wallet.base_currency))
